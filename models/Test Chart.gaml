      ...
      // FIND initial RATIO + speed at start of each road
        ...
        // Handle special road blockage, re-calculate things
          ...
          if (is_in_blocked_road = false) {
            if (is_on_node or road_before_change != current_road) {
...
//        write "new? already cant find path wont attempt to find " + self;
        return;
      }
...
//        write "I am " + self;
        ...
        if (shortest_path != nil and shortest_path.vertices[length(shortest_path.vertices) - 1] != dest) {
          write "Fixed no edges connecting source";
          cant_find_path <- true;
          speed <- 0 #m / #s;  
        }
